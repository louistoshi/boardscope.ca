#!/usr/bin/env node
// Debug script v8 - correct parser: fields starting with 0d0a AND length > 2 are NET fields
const fs = require('fs');
...
// NEW LOGIC: A field is a NET field if:
// 1. It has 0x0d0a NOT at position 0 (normal net field), OR
// 2. It starts with 0x0d0a AND has length > 2 AND has another 0x0d0a later (empty net name)
// Otherwise it's a COORD field
function isNetField(f) {
  if (f.length < 3) return false;
  
  // Case 1: Starts with 0x0d0a - check if there's another 0x0d0a later
  if (f[0] === 0x0d && f[1] === 0x0a) {
    for (let j = 2; j < f.length - 1; j++) {
      if (f[j] === 0x0d && f[j+1] === 0x0a) {
        return true; // Empty net name net field
      }
    }
    return false; // Just a coord field that happens to start with 0x0d0a
  }
  
  // Case 2: Has 0x0d0a NOT at position 0
...
// Parse with pattern: net, Y, X, side
console.log('--- Corrected parser (net fields start with 0d0a AND have another 0d0a) ---');
let lastNetName = '';
const pins = [];
let i = 0;
let parsed = 0;
