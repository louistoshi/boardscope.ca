#!/usr/bin/env node
// Debug script v5 - parse sequentially: net, Y, X, side, net, Y, X, side...
const fs = require('fs');
...
// Check if field is a net field (contains 0x0d0a NOT at position 0, with cipher bytes before it)
...
console.log('--- Sequential parsing (net, Y, X, side pattern) ---');
let lastNetName = '';
const pins = [];
let i = 0;
let parsed = 0;
...
while (i < fields.length && parsed < 30) {
  ...
  if (isNetField(f)) {
    ...
    if (info && info.pinIdx >= 0) {
      ...
      console.log(`  Y field: ${yField ? Array.from(yField).map(b=>b.toString(16).padStart(2,'0')).join(' ') : 'N/A'}`);
      console.log(`  X field: ${xField ? Array.from(xField).map(b=>b.toString(16).padStart(2,'0')).join(' ') : 'N/A'}`);
      console.log(`  Side field: ${sideField ? Array.from(sideField).map(b=>b.toString(16).padStart(2,'0')).join(' ') : 'N/A'}`);
